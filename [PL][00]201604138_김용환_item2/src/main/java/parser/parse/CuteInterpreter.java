...
public class CuteInterpreter {
	...
	private Node runList(ListNode list) {
		...
		{
			Node result = lookupTable(list.car().toString());	//해시맵에 key값에 따른 value값을 저장한다.
			
			//일반적인 키워드인경우 tail이 비어있기 때문에 비어있다면 그대로 반환한다.
			if(list.cdr().equals(ListNode.ENDLIST))
			{
				return result;
			}else {	//lambda인 경우 뒤의 원소가 있기 때문에 lambda 수식과 정수원소를 리스트로 만들어 수식을 수행한다.
				Node expr = ListNode.cons(((ListNode) result).car(), list.cdr());
				return runExpr(expr);
			}
		}
		
		//lambda인지 검사한다.
		if(list.car() instanceof ListNode && list.cdr()!=ListNode.ENDLIST)
		{
			if(((ListNode) list.car()).cdr().equals(ListNode.ENDLIST))
			{
				return list;
			}else {
				String key = ((ListNode) ((ListNode) list.car()).cdr().car()).car().toString();
				insertTable(key, list.cdr());
	...
	private Node runFunction(FunctionNode operator, ListNode operand) {
		switch (operator.funcType) {
			...
			case DEFINE:
				String key = operand.car().toString();			//key값 저장
				
				//body부분이 lambda이면 해시맵에 그대로 저장한다.
				if(((ListNode) operand.cdr().car()).car().toString().equals("LAMBDA"))
				{
					table.put(key, operand.cdr());
				}else {	//lambda이외의 body이면 runExpr을 한번 수행하고 결과를 저장한다.
	...
	{
		Node expr = table.get(id);
		Node result;
		if(expr instanceof IntNode || expr instanceof BooleanNode)
		{
			result = expr;
			return result;
		}
		
		//expr이 lambda 수식인지 검사한다.
		if(((ListNode) ((ListNode) expr).car()).car().toString().equals("LAMBDA"))
		{
			result = expr;
		}else {
			result = runExpr(expr);
